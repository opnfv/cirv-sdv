...
def capability_check():
    v1 = kube_api()
    cap = open("./example/sec_check_yaml/security_capability.yaml")
...
def privilege_check():
    v1 = kube_api()
    cap = open("./example/sec_check_yaml/security_privileges.yaml")
...
def host_network_check():
    ...
    cap = open("./example/sec_check_yaml/security_host_network.yaml")
...
def host_path_vol_check():
    v1 = kube_api()
    cap = open("./example/sec_check_yaml/security_host_path_vol.yaml")
...
def k8s_api_conn_check():
    v1 = kube_api()
    cap = open("./example/sec_check_yaml/security_connection.yaml")
    security_connectivity = yaml.load(cap, Loader= yaml.FullLoader)
    ...
    try:
        resp = v1.create_namespaced_pod(body=security_connectivity, namespace='default')
        
        time.sleep(3)
        

        cmd = ['wget', 'http://localhost:8001/api/']
        kube_exec(resp, cmd)

        cmd1 = ['cat','api']
        response = kube_exec(resp, cmd1)
